...
{
    /**
     * Test for getCronHour
     */
    public function testGetCronHour_Should_Return_Cron_Hours_When_Always()
    {
        //arrange
        $expected = array(
...
//        array('name' => 'every twelve', 'value' => '0,12'),
        ...
        //act
        $cron = new Cron();
        $cron_hour = $cron->getCronHour();

        //assert
        $this->assertEquals($expected, $cron_hour);
    }

    /**
     * Test for getCronDay
     */
    public function testGetCronDay_Should_Return_Cron_Days_When_Always()
    {
        //arrange
        $expected = array(
        ...
        //act
        $cron = new Cron();
        $cron_days = $cron->getCronDay();

        //assert
        $this->assertEquals($expected, $cron_days);
    }

    /**
     * Test for getCronMonth
     */
    public function testGetCronMonth_Should_Return_Cron_Months_When_Always()
    {
        //arrange
        $expected = array(
...
//        array('name' => 'every six month', 'value' => '1,7'),
        ...
        //act
        $cron = new Cron();
        $cron_month = $cron->getCronMonth();

        //assert
        $this->assertEquals($expected, $cron_month);
    }

    /**
     * Test for getCronWeek
     */
    public function testGetCronWeek_Should_Return_Cron_Week_When_Always()
    {
        //arrange
        $expected = array(
            array('name' => 'every day', 'value' => '*'),
...
//        array('name' => 'every weekend', 'value' => '6,0'),
        ...
        //act
        $cron = new Cron();
        $cron_week = $cron->getCronWeek();

        //assert
        $this->assertEquals($expected, $cron_week);
    }
}
